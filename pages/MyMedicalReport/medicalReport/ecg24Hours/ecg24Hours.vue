<template>
	<view class="home">
		<!-- 首部轮播图 -->
		<view class="header">
			<text class="header-text">24小时动态心电图（ECG）</text>
		</view>
		...
		<view class="box">
			...
			<view class="item" v-for="(value,key,index) in this.Array" :key="key">
				...
				<view class="item_detail">
					<text>平均心率:{{value.average}} (次/分)</text>
				</view>
			...
			<view class="item">
				<button type="primary" @click="submit">新增心电记录</button>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			getData() {
				const url = 'http://1.117.222.119/v1/user/mr24hoursecg?limit_count=0'
